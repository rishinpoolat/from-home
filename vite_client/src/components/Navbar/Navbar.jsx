...
export default function Navbar() {
  ...
  const quantity = cart[0]?.products?.length;
    ...
    <nav className="navbar ai-c df">
      ...
      <div className="nav-links ai-c df">
        ...
        {user || res?.user ?
          ...
          {path !== "user" && <Link className="link" to={`/user/${user?.result?._id || res?.user?._id}`}>
            <img
              className="user-pic df mr-0"
              src={
                user?.result?.imageUrl ||
                `https://avatars.dicebear.com/api/initials/${user?.result?.name || res?.user?.name})}.svg`
              }
